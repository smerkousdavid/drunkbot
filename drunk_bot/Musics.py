...
'''
This file was copied and then modified from
soundscrape.py the repository can be found
here https://github.com/Miserlou/SoundScrape/blob/master/soundscrape/soundscrape.py
'''
...
def download_raw(url_mp3, output="/songs/", system: System=None):
    try:
        decoded = str(unquote(url_mp3))
        name = re.sub('[^0-9a-zA-Z-+ ]+', '', decoded[decoded.rfind("/")+1:]).replace('+', ' ')
        if isfile(output + name):
            remove(output + name)
        urlretrieve(url_mp3, output + name)
        if not name.endswith(".mp3"):
            out_name = output + (name[:name.rfind(".")]) + ".mp3"
            if isfile(out_name):
                remove(out_name)
            system.ffmpeg("-i %s %s" % (output + name, out_name))
    except Exception:
        raise TypeError("Error downloading requested song")


def process_text(text: str, songs: int = 1, system: System=None, sync: Sync=None):
    sync.song = True
    back_path = dirname(__file__)
    chdir(back_path + "/songs/")

    if "http" in text and "://" in text:
        print("Downloading raw format")
        download_raw(text.strip().replace('\n', ''), dirname(__file__) + "/songs/", system)
    ...
    vargs['artist_url'] = text
    vargs['num-tracks'] = songs
...
def get_soundcloud_api_playlist_data(playlist_id):
    ...
    url = "https://api.soundcloud.com/playlists/%s?representation=full&client" \
          "_id=02gUJC0hH2ct1EGOcYXQIzRFU91c72Ea&app_version=1467724310" % (
              playlist_id)
...
def get_mixcloud_data(url):
    ...
    stream_server = \
        request.text.split('m-p-ref="cloudcast_page" m-play-info="')[1].split('" m-preview="')[1].split(
            '.mixcloud.com')[0]
